...
if (isset($_POST['simpan_keluar'])) {
    ...
    if ($id_barang == "" || $qty <= 0) {
        ...
        $cek_stok = mysqli_query($conn, "SELECT stok FROM barang WHERE id = '$id_barang'");
        $data_stok = mysqli_fetch_assoc($cek_stok);

        if ($data_stok['stok'] < $qty) {
            $pesan_gagal = "Stok tidak cukup! Sisa: " . $data_stok['stok'];
        } else {
            $simpan_riwayat = mysqli_query($conn, "INSERT INTO barang_keluar (id_barang, tanggal, qty, alasan) VALUES ('$id_barang', '$tanggal', '$qty', '$alasan')");
            $update_stok = mysqli_query($conn, "UPDATE barang SET stok = stok - '$qty' WHERE id = '$id_barang'");
            if ($simpan_riwayat && $update_stok) $pesan_sukses = "Stok berhasil dikurangi!";
...
<div class="d-flex">
    <div class="sidebar p-3" style="width: 250px;">
        ...
        <a href="barang_keluar.php" class="active"><i class="bi bi-arrow-up-square me-2"></i> Barang Keluar</a>
        ...
        <div class="card mt-3" style="max-width: 600px;">
            <div class="card-body border-top border-danger border-3">
                <form action="" method="POST">
                    <div class="mb-3"><label>Tanggal Keluar</label><input type="date" class="form-control" name="tanggal" value="<?= date('Y-m-d'); ?>" required></div>
                    ...
                    <div class="mb-3"><label>Alasan</label>
                        <select class="form-select" name="alasan">
                            ...
                            <option value="Pemakaian Sendiri">Pemakaian Sendiri</option>
                        </select>
                    </div>
                    <button type="submit" name="simpan_keluar" class="btn btn-danger w-100">Simpan</button>
                </form>
            </div>
        </div>

        <div class="card mt-4">
            <div class="card-header text-danger">Riwayat Terakhir</div>
            <div class="card-body p-0">
                <table class="table table-striped mb-0">
                    <thead><tr><th>Tanggal</th><th>Barang</th><th>Qty</th><th>Alasan</th></tr></thead>
                    <tbody>
                        <?php
                        $history = mysqli_query($conn, "SELECT barang_keluar.*, barang.nama_barang FROM barang_keluar JOIN barang ON barang_keluar.id_barang = barang.id ORDER BY barang_keluar.id DESC LIMIT 5");
                        while($h = mysqli_fetch_assoc($history)){
                            echo "<tr><td>$h[tanggal]</td><td>$h[nama_barang]</td><td class='text-danger'>-$h[qty]</td><td>$h[alasan]</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
